...
const TITLE = 'Unlock the full application report'
const SUBTITLE =
  'Get the complete CV analysis, ATS keywords, tailored cover letter, and PDF export for this job application.'
...
export function ConversionUpgradeModal({
  ...
  const handleUnlockThisReport = useCallback(async () => {
  ...
  const headline =
    variant === 'quota_monthly' ? "You've hit your Monthly Pro limit" : TITLE
  ...
  return (
    ...
    >
      <div className="relative max-h-[min(90vh,720px)] w-full max-w-lg overflow-y-auto rounded-[28px] border border-slate-700/90 bg-gradient-to-b from-slate-900 to-slate-950 p-6 shadow-[0_0_80px_rgba(34,211,238,0.14)] sm:max-w-xl sm:p-8">
        ...
        <h2 id="jobfit-conversion-modal-title" className={`${eyebrow ? 'mt-3' : ''} text-xl font-bold tracking-tight text-slate-50 sm:text-2xl`}>
          {headline}
        </h2>
        ...
        {variant === 'quota_monthly' ? (
          <div className="mt-8 flex flex-col gap-3">
            ...
            >
              {subscribeMonthlyBusy ? 'Opening Checkout…' : 'Upgrade monthly'}
            </button>
            <button
              type="button"
              onClick={onClose}
          ...
          <div className="mt-8 grid gap-4 sm:grid-cols-2">
            <div className="flex flex-col rounded-2xl border border-violet-400/25 bg-violet-500/[0.07] p-5">
              ...
              <p className="mt-2 text-lg font-semibold text-slate-50">Pro Report</p>
              {appliedPromo ? (
                <>
                  ...
                  <p className="mt-2 text-[11px] leading-snug text-emerald-200/85">
                    {appliedPromo.code} · −{appliedPromo.percentOff}% · Stripe Checkout shows the discounted total before you
                    pay.
                  </p>
                </>
              ) : (
                <p className="mt-1 text-2xl font-bold tabular-nums text-violet-100">€{PRICE_PRO_REPORT_EUR}</p>
              )}
              <p className="mt-3 flex-1 text-xs leading-relaxed text-slate-400">
                Full breakdown for this application only — ATS checklist, exports, and cover letter tooling.
              </p>
              <div className="mt-4">
                <ProReportPromoBox
                  key={analysisId ?? '_'}
                  compact
                  disabled={!analysisId || proBusy || proReportCreditsCount > 0}
                  onApplied={setAppliedPromo}
                />
              </div>
              <button
                type="button"
                disabled={!analysisId || proBusy}
                onClick={() => void handleUnlockThisReport()}
            ...
            <div className="flex flex-col rounded-2xl border border-cyan-400/25 bg-slate-950/80 p-5 shadow-inner">
              <p className="text-[11px] font-semibold uppercase tracking-[0.16em] text-cyan-200/90">Subscription</p>
              <p className="mt-2 text-lg font-semibold text-slate-50">Monthly Pro</p>
              <p className="mt-1 text-2xl font-bold tabular-nums text-cyan-100">
                €{PRICE_MONTHLY_PRO_EUR}
                <span className="text-sm font-semibold text-slate-500">/month</span>
              </p>
              <p className="mt-3 flex-1 text-xs leading-relaxed text-slate-400">
                Higher monthly quota plus subscriber features — billed securely via Stripe.
              </p>
              <button
                type="button"
                disabled={subscribeMonthlyBusy}
